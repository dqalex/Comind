...
`gateway-client.ts`（浏览器直连）和 `gateway-proxy.ts`（服务端代理）两个 Gateway 客户端独立定义 RPC 方法名字符串。v0.2.7 之前 `gateway-proxy.ts` 使用了错误的方法名（`cron.create`/`cron.delete` 而非正确的 `cron.add`/`cron.remove`），因为两个文件无共享常量引用。
...
- `server_proxy` 模式下创建/删除 cron job 失败（已在 v0.2.7 修复）
